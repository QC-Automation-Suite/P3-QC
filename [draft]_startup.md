...
- UiPath Studio ([_installation link_](https://docs.uipath.com/installation-and-upgrade/docs/studio-install-studio))
...
2. _QC Report Helper_:
    a. [VERIFY] `Excel Sheets` : Storage Bucket
...
c. `Report Helper Queue Trigger` : Trigger of _Queue_ type.
    * Configuration:
...
3. _P0 Analyzer_:
a. `Repos` : Storage Bucket
b. `Reports` : Storage Bucket
c. `OutputFiles` : StorageBucket
d. `EmailCred` : Asset of _Credential_ type.
    * Configuration: Enter the email id and password of the gmail account that is to be used to send a copy of the final PDF report generated by this automation. [unsecure app access].
e. [VERIFY] `TrainerP0Wiki` : Asset of _Text_ type.
